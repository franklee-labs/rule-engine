...
public class Or extends Relation {
    ...
    public PathGroup calculate() {
        if (this.getChildren().isEmpty()) {
            throw new RuntimeException("Or node mush have at least one node.");
        }
        ...
        for (int i = 1; i < this.getChildren().size(); i++) {
            ...
            previousPaths.paths().forEach(pathGroup::addPath);
            currentPaths.paths().forEach(pathGroup::addPath);
